...
def plot_label_distribution(df, col, title, save_path):
    ...
    ax = sns.barplot(data=plot_df, x='label', y='count', palette='Set2')
...
def plot_star_vs_sentiment(df, save_path):
    ...
    plt.title('Tỷ lệ cảm xúc theo số sao (%)')
    plt.xlabel('Cảm xúc')
    plt.ylabel('Số sao')
    plt.tight_layout()
    plt.savefig(_prepare_save_path(save_path), dpi=150)
...
def plot_sentiment_by_cluster(df, cluster_col, sentiment_col, save_path):
    ...
    plt.title('Tỷ lệ cảm xúc theo từng cụm chủ đề')
    plt.xlabel('Cụm')
    plt.ylabel('Tỷ lệ (%)')
...
def plot_top_words(df, text_col, sentiment_col, save_path):
    ...
    for ax, sentiment in zip(axes, sentiments):
        ...
        sns.barplot(data=words, y='word', x='count', ax=ax, palette='viridis')
        ax.set_title(f'Top 20 từ - {sentiment}')
